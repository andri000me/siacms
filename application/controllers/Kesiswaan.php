...
class Kesiswaan extends CI_Controller {
	...
	function __construct(){
		...
		if (($this->session->userdata('jabatan') != 'Kesiswaan')&&($this->session->userdata('jabatan') != 'Superadmin')&&($this->session->userdata('jabatan') != 'Kurikulum')&&($this->session->userdata('jabatan') != 'Guru')&&($this->session->userdata('jabatan') != 'Pegawai')&&($this->session->userdata('jabatan') != 'Kepsek')){
			$this->session->set_flashdata("warning",'<script> swal( "Maaf Anda Tidak Berhak!" ,  "Silahkan Login dengan Akun Anda" ,  "error" )</script>');
	...
	//index punya modul kesiswaan (ppdb dan distribusi)
	public function index(){
		$data['nama'] = $this->session->Nama;
		$data['foto'] = $this->session->foto;
		$this->template->load('kesiswaan/dashboard', 'kesiswaan/ppdb/admin/view_dashboard_admin', $data);
		$this->load->model('distribusi/mod_pengaturan_kelas');
	...
	public function distribusi_reg(){
		...
		$data['tabel_pengaturan_kelas_reg'] = $this->mod_pengaturan_kelas->get();
	...
	public function pilih_pembagian_tambahan() {
		$data['nama'] = $this->session->Nama;
		$data['foto'] = $this->session->foto;
		$tipe = $this->input->post('btntipe');
		...
		$jenjang_kls_tambahan = $this->input->post('jenjang_kls_tambahan');
			...
			redirect('kesiswaan/excel_import/'.$jenjang_kls_tambahan);
	}

	public function excel_import($jenjang_kls_tambahan)
	 {
	 	$data['nama'] = $this->session->Nama;
		$data['foto'] = $this->session->foto;
		$data['jenjang_kls_tambahan'] = $jenjang_kls_tambahan;

		$this->load->model('distribusi/mod_kelas_tambahan');

		$data['kelas_tambahan'] = $this->mod_kelas_tambahan->getjenjang($jenjang_kls_tambahan);
	...
	public function hsl_pembagian_kuartil($value=''){
		...
		// foreach ($this->db->get('tabel_pengaturan_kelas_reg')->result() as $tabel) 
  //        { 
  //        	$settingtabel[$tabel->nama_kolom] = $tabel->nilai;
  //        	$settingtabelberkas[$form->nama_kolom] = $tabel->atribut;
  //        }
  //        $data['settingtabel'] = $settingtabel;
  //        $data['settingtabelberkas'] = $settingtabelberkas;

		redirect('kesiswaan/proses_pembagian_kuartil/'.$jenjang);
	}

	public function penempatan_kelas_tambahan($value=''){
		$data['nama'] = $this->session->Nama;
		$data['foto'] = $this->session->foto;

		$jenjang_kls_tambahan = $this->input->post('jenjang_kls_tambahan');

		$this->form_validation->set_rules('id_kelas_tambahan', 'Kelas', 'required');
		$this->form_validation->set_rules('pilih[]', 'Siswa', 'required');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$id_kelas_tambahan = $this->input->post('id_kelas_tambahan'); //3;

			$this->load->model('distribusi/mod_siswa_kelas_tambahan');
			
			foreach ($_POST['pilih'] as $id_siswa_excel) {
			 	$this->mod_siswa_kelas_tambahan->insert(array('id_kelas_tambahan'=>$id_kelas_tambahan, 'id_siswa_excel' => @$id_siswa_excel));
			}
			$this->session->set_flashdata('success', 'Data Berhasil Disimpan!');
		}
	...
	public function buat_kelas_reguler_berjalan(){
		...
		$data_kelas_reguler_berjalan = $this->mod_kelas_reguler_berjalan->getjoin();
		$tahun_ajaran       = $this->Mod_tahunajaran->getaktif()->id_tahun_ajaran;
		
		foreach ($data_kelas_reguler as $key => $kelas) {
	...
	public function proses_pembagian_kuartil($jenjang)
	{
		...
		foreach ($this->db->get('tabel_pengaturan_kelas_reg')->result() as $tabel) 
         { 
         	$settingtabel[$tabel->nama_kolom] = $tabel->nilai;
         	$settingtabelberkas[$tabel->nama_kolom] = $tabel->atribut;
         }
         $data['settingtabel'] = $settingtabel;
         $data['settingtabelberkas'] = $settingtabelberkas;
	...
	public function distribusi_tam(){
		...
		$data_siswa_kelas_tambahan = $this->mod_kelas_tambahan->getjoin();
		...
		$data['data_kelas_tambahan'] = $data_siswa_kelas_tambahan;
	...
	public function pengacakan_tambahan(){
	   ...
	   //      }
		...
		//redirect('kesiswaan/hasil_pembagian_tambahan');
	      redirect('distribusi/excel_import');
	}
        ...
        foreach ($this->db->get('tabel_pengaturan_kelas_reg')->result() as $tabel) 
         { 
         	...
         	{ //input text dari admin
	         	if ($this->input->post('nilai'.$tabel->id_pengaturan_kelas_reg) == "1") 
	         	{
	         		$nilai = 1;
	         		$atribut = $this->input->post('atribut'.$tabel->id_pengaturan_kelas_reg);
	         	} 
	         	else 
	         	{ 
	         		$nilai = 0; 
				...
				if ($this->input->post('atribut'.$tabel->id_pengaturan_kelas_reg) != "") 
				{
					$arrdata['atribut'] = $this->input->post('atribut'.$tabel->id_pengaturan_kelas_reg); 
				}
				
			}

			$this->load->model('distribusi/mod_pengaturan_kelas');
			$this->mod_pengaturan_kelas->update($arrdata, $tabel->id_pengaturan_kelas_reg);
            $i=$i+1;
         }
